...
class Bot(Okx):
    ...
    а методы взаимодействия с биржей OKX наследуются из класса OKX (декомпозиция)
    при желании можно наследоваться от класса, где будет реализовано взаимодействие
    или с другой биржей, или с другой секцией биржи (фьючерсы)
    ...
    def is_cross(self):
        ...
        if close.empty:
            logger.error(f"No close prices data available for {self.symbol}")
            return 0
        ...
        if fast[-1] > slow[-1] and fast[-2] < slow[-2]:
            ...
            r = -1  # crossunder быстрая сверху вниз
